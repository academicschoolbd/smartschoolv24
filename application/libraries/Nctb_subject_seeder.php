...
class Nctb_subject_seeder
{
    ...
    public function seedForBranch($branchId, array $options = [])
    {
        ...
        foreach ($this->getCatalog()['levels'] as $levelKey => $level) {
            ...
            foreach (($level['subjects'] ?? []) as $row) {
                ...
                $ins++;
                $result['subjects'][] = [
                    'name' => $name, 'code' => $code, 'type' => $type, 'level_key' => $levelKey,
                ];
            }
            $result['inserted'] += $ins;
            $result['skipped']  += $skp;
     ...
     */
    public function seedClassesForBranch($branchId)
    {
    ...
    {
        $classes    = $this->seedClassesForBranch($branchId);
        ...
        $exam       = $this->seedExamDefaultsForBranch($branchId);
        $exams      = $this->seedStarterExamsForBranch($branchId);
     ...
     */
    public function seedExamDefaultsForBranch($branchId)
    {
        $out = [
        ...
        if ($db->table_exists('exam_term')) {
            try {
                ...
                if ($sessionId > 0) {
                    ...
                    foreach ((array)$this->CI->config->item('nctb_exam_terms') as $term) {
     ...
     * @return array{inserted:int, skipped:int}
     */
    public function seedStarterExamsForBranch($branchId)
    {
        $out = ['inserted' => 0, 'skipped' => 0];
        $branchId = (int)$branchId;
        if ($branchId <= 0) return $out;
        ...
        try {
            ...
            foreach ((array)$this->CI->config->item('nctb_starter_exams') as $ex) {
                ...
                // Resolve term id (also tolerate Bangla equivalents via
                // the Bangla alias map below).
                ...
                // Resolve mark-distribution ids; only keep ones that exist.
                $mdIds = [];
    ...
    private function _catalogClassMap()
    {
        ...
        if ($this->ensureCatalogTable() && $this->countCatalogRows() > 0) {
            foreach ($this->getCatalogRows(['active_only' => true]) as $r) {
                ...
                if (!empty($classes)) $map[$key] = $classes;
            }
    ...
    private function _key($name, $code, $type)
    {
        ...
        return $name . '|' . trim((string)$code) . '|' . trim((string)$type);
    }
}
